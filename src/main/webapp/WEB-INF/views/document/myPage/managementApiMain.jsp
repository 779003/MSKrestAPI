...
<div id="container">
	...
	<ul class="tab">
		<c:if test="${fn:length(myApps) >= 1}">
			<c:forEach var="app" items="${myApps}" varStatus="status">
				<li <c:if test="${status.index eq 0}">class="current"</c:if> data-tab="tab${status.index}">
	...
	<c:choose>
		<c:when test="${fn:length(myApps) == 0}">
			<div id="tab6" class="tabcontent current">
				<h3>사용중인 API가 없습니다.</h3>
				<p>API를 사용하시려면 하단의 오픈API신청 버튼을 눌러 어플리케이션을 생성해 주세요.</p>
			</div>
		</c:when>
		<c:otherwise>
			<c:forEach var="app" items="${myApps}" varStatus="status">
				...
				<div id="tab${status.index}" class="tabcontent  <c:if test="${status.index eq 0}">current</c:if>" >
						...
						<table class="appInfoTable" style="width: 50%;">
							...
							<tr>
								...
								<td>
									...
									<fmt:formatDate value="${dateString}" pattern="yyyy년 MM월 dd일 HH시 mm분" /></td>
							</tr>
							<tr>
								<th>사용 여부</th>
								<td>
					...
					<div align="center" style="padding-top: 10px">
						<table class="appInfoTable" style="width: 100%;">
							...
							<tr>
								<th colspan="2">오픈 API 일일 사용량 
									<span style="color: red" id='usageDesc${status.index}'></span>
								</th>
							</tr>
							<tr>
					...
					<div align="right">
						<div class="btn-group" role="group">
							<button type="button" class="btn btn-default"
								onclick="javascript:location.href ='/apps/modify/${app.api_key}'">수정하기</button>
							<button type="button" class="btn btn-default"
								onclick="javascript:location.href ='/apps/delete/${app.api_key}'">삭제하기</button>
						</div>
					</div>
				</div>
				<script>
...
<div class="alert alert-success" role="alert">
	<a href="/apps/register">오픈 API 신청</a>
</div>
